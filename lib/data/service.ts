...
export const BACKDESK_REASONS = {
  eyebrow: 'OurDeskが選ばれる理由',
  headline: 'すべて見える\nだから任せられる',
  sub:
    '稼働・請求・権限を一元管理し、任せっぱなしにしない体制を実現します。',
  items: [
    '稼働はすべてシステムに記録：実施内容・時間・履歴が残り、口頭ベースになりません。',
    '稼働と請求が連動：「何に対する請求か」が分かる状態で管理できます。',
    'いつでも状況を確認できる：実施内容・稼働時間・履歴をいつでも確認できます。',
...
export const SERVICES_HEAD = {
  ...
  body: 'OurDeskは、バックオフィスの実務から、調査・資料作成・採用運用・広報支援・IT運用まで幅広くサポートします。',
  footer:
