    ...
    return (
        <section className="relative z-[7] bg-[#fffdf5] py-20 md:py-32 px-4 md:px-6" aria-label="キャリアの描き方">
            ...
            <BlobDecoration
                shape="T"
                drift="float-e"
                fill="rgba(235, 180, 178, 0.2)"
                className="w-[75vw] h-[85vw] md:w-[37vw] md:h-[43vw] max-md:!top-[-22vw] max-md:!right-[-24vw]"
                style={{ top: '-14vw', right: '-12vw' }}
            />
            <div className="max-w-6xl mx-auto relative z-10">
                <div className="text-center mb-12 md:mb-20">
                    ...
                    <div className="mb-4">
                        <HandwrittenLine variant={5} color="#FDD000" width={100} align="center" />
                    </div>
                        ...
                        >
                            <div
                                className={cn(
                                    ...
                                    path.id === '02'
                                        ? 'bg-gradient-to-br from-[#F08300] to-[#FDD000]'
                                        : 'bg-gradient-to-br from-[#FDD000] to-[#F08300]'
                                )}
                            >
                                {path.id}
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-amber-50/80 to-orange-50/80 rounded-xl p-4 md:p-6 mb-6 md:mb-8">
                                <p className="text-sm text-gray-500 mb-2">現在</p>
                                {path.current.map((c, i) => (
                                    <p
                                        key={c}
                                        className={cn('text-sm md:text-base text-gray-700 leading-relaxed text-pretty', i < path.current.length - 1 && 'mb-1')}
                                    >
                                        {c}
                                    </p>
                                ))}
                            ...
                            <div className="space-y-3 md:space-y-4">
                                ...
                                {path.steps.map((step, i) => (
                                    <div key={i} className="flex items-start gap-3">
                                        <div className="size-2 rounded-full bg-[#F08300] mt-2 flex-shrink-0" />
                                        <p className="text-sm md:text-base text-gray-600 leading-relaxed text-pretty">{step}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
